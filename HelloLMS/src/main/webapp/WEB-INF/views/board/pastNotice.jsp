...
<div class="container">
	...
	<div class="pagination" style="text-align: center">
			<ul style="float: center;">
				<c:if test="${cPage > 10}">
					<li><a href="/board/pastNotice?cPage=${prePage}">PREV</a></li>
				</c:if>
				<c:forEach var="page" begin="${sPage}" end="${(sPage + pPageCnt) - 1}">
					<c:if test="${page<=tPage}">
						<c:if test="${page==cPage}">
							<li class="active"><a href="/board/pastNotice?cPage=${page}">${page}</a></li>
						</c:if>
						<c:if test="${page!=cPage}">
							<li><a href="/board/pastNotice?cPage=${page}">${page}</a></li>
						</c:if>
					</c:if>
				</c:forEach>
				
				<c:if test="${nPage < tPage}">
						<li><a href="/board/pastNotice?cPage=${nPage}">NEXT</a></li>
