...
<div class="container">
	...
	<table class="table table-bordered">
		...
		<c:forEach items="${rep}" var="rep" varStatus="i">
			...
			<tr>
				<td><div class="span1">${(cPage-1)*10+i.count}</div></td>
		...
		<tr>
			<td><div class="span1"><button type="button" class="btn btn-default btn-sm" onclick="location.href='/board/qna/write'">작성</button></div></td>
	...
	<div class="pagination" style="text-align: center">
			<ul style="float: center;">
				<c:if test="${cPage > 10}">
					<li><a href="/board/ad_qna?cPage=${prePage}">PREV</a></li>
				</c:if>
				<c:forEach var="page" begin="${sPage}" end="${(sPage + pPageCnt) - 1}">
					<c:if test="${page<=tPage}">
						<c:if test="${page==cPage}">
							<li class="active"><a href="/board/ad_qna?cPage=${page}">${page}</a></li>
						</c:if>
						<c:if test="${page!=cPage}">
							<li><a href="/board/ad_qna?cPage=${page}">${page}</a></li>
						</c:if>
					</c:if>
				</c:forEach>
				
				<c:if test="${nPage < tPage}">
						<li><a href="/board/ad_qna?cPage=${nPage}">NEXT</a></li>
